...
export const userService = {
  ...
  getRegistrationForm: async (eventId) => {
    try {
      ...
      if (
        ...
        const matchingForm = response.data.data.find(
          (form) => form.eventId === eventId
        );
      ...
      handleApiError(
        error,
        "Terjadi kesalahan saat mengambil formulir pendaftaran"
      );
    }
  },

  getEventHistories: async () => {
    try {
      ...
      // Gunakan httpClient dengan error handling yang lebih baik
      const response = await httpClient.get(`${API_URL}/events/histories`);
      return response.data;
    } catch (error) {
      console.error("Error fetching event histories:", error);

      // Return empty data untuk mencegah crash
      if (error.response?.status === 500) {
        ...
        return {
          ...
          data: [],
          message:
            "Server sedang bermasalah, riwayat pendaftaran tidak dapat dimuat",
        };
      }

      handleApiError(error, "Gagal mengambil riwayat pendaftaran");
    }
  },
  ...
  getRegistrationStatus: async (eventId) => {
    try {
      ...
      if (!token) {
        console.warn(
          "No auth token available for checking registration status"
        );
        return { registered: false };
      }

      const response = await httpClient.get(
        `${API_URL}/events/${eventId}/registration-status`
      );
      return response.data;
    } catch (error) {
